...
// you see some number of random cubes

// record information from each game
...
const input = `Game 1: 1 green, 2 blue; 13 red, 2 blue, 3 green; 4 green, 14 red
...
const processInput = (str) => {
  const possibilities = { red: 12, green: 13, blue: 14 };
  const obj = {};

  const gamesArray = str.split("\n");

  gamesArray.map((game, id) => {
    const name = game.split(":");
    const gameId = name[0].split(" ");
    obj[gameId[1]] = name[1];
  });
  ...
  for (let i in obj) {
    const gameArray = obj[i].split(";");

    const arr = [];

    gameArray.map((game, id) => {
      const gameObj = {};
      const cube = game.split(",");

      cube.map((c) => {
        const split = c.split(" ");
        gameObj[split[2]] = split[1];
      });
  ...
  for (let outerKey in obj) {
    for (let innerKey in obj[outerKey]) {
      if (obj[outerKey][innerKey].red > 12) {
        ids.push(outerKey);
        break;
      }

      if (obj[outerKey][innerKey].green > 13) {
        ids.push(outerKey);
        break;
      }

      if (obj[outerKey][innerKey].blue > 14) {
        ids.push(outerKey);
        break;
      }
    }
  ...
  const winningGames = Object.keys(obj).filter((id) => !ids.includes(id));

  let sum = 0;
  winningGames.forEach((id) => {
    sum += Number(id);
  });
