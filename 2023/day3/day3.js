...
const sumPartNumbers = (input) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    for (let j = 0; j < currArr.length; j++) {
      const currVal = currArr[j];

      if (!isNaN(currVal)) {
        number = `${number}${currVal}`;

        if (prevArr && prevArr[j - 1] && isSymbol(prevArr[j - 1])) {
          isAdjacent = true;
        }
        if (prevArr && prevArr[j + 1] && isSymbol(prevArr[j + 1])) {
          isAdjacent = true;
        }
        if (prevArr && prevArr[j] && isSymbol(prevArr[j])) {
          isAdjacent = true;
        }
        if (currArr && currArr[j - 1] && isSymbol(currArr[j - 1])) {
          isAdjacent = true;
        }
        if (currArr && currArr[j + 1] && isSymbol(currArr[j + 1])) {
          isAdjacent = true;
        }
        if (nextArr && nextArr[j - 1] && isSymbol(nextArr[j - 1])) {
          isAdjacent = true;
        }
        if (nextArr && nextArr[j + 1] && isSymbol(nextArr[j + 1])) {
          isAdjacent = true;
        }
        if (nextArr && nextArr[j] && isSymbol(nextArr[j])) {
          isAdjacent = true;
        }
      }
...
console.log(sumPartNumbers(testInput)); // 4361
console.log(sumPartNumbers(corner1));
console.log(sumPartNumbers(corner2));
