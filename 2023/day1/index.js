const getCalibrationValues = (string) => {
  const strWithCommas = string.replace(/(\r\n|\r|\n)/g, ",");
  const arr = strWithCommas.split(",");
  arrOfDigits = [];
  ...
  arr.forEach((a) => {
    ...
    if (collection.length === 1) {
      arrOfDigits.push(parseInt(`${collection[0]}${collection[0]}`));
    } else {
      arrOfDigits.push(
        parseInt(`${collection[0]}${collection[collection.length - 1]}`)
      );
    }
  });
...
// getCalibrationValues(`rhqrpdxsqhgxzknr2foursnrcfthree
// 2bmckl
// four95qvkvveight5
